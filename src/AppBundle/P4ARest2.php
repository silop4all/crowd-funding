...
//    }
    ...
    public static function makeProject($em, $session, $entity, $doctrine) {
        $u = $doctrine
        ->getRepository(\AppBundle\Entity\User::class)
        ->find($session->get('user_info')->sub);

    if (!$u) {
        throw $this->createNotFoundException(
            'No product found for id '.$u
        );
    }
	//echo print_r($session->get('user_info'), true);
	//echo print_r($session->get('user_info')->sub, true);
	$entity->setUser($u);
